    ...
    <div class="max-w-wide mx-auto mt-8 px-4 sm:px-6 lg:px-8 py-4">
      ...
      <div class="flex flex-col md:flex-row items-center justify-between gap-2 text-sm text-foreground-secondary">
        ...
        <p class="inline-flex flex-wrap items-center gap-x-2 gap-y-1 justify-center sm:justify-start">
          <span>Copyright &copy; {{ copyrightYears }} 
            <template v-if="siteConfig.author.url">
              <a :href="siteConfig.author.url" target="_blank" rel="noopener noreferrer" class="border-b border-dashed border-border-secondary hover:text-foreground transition-colors">{{ authorName }}</a>
            </template>
            <template v-else>
              {{ authorName }}
            </template>
